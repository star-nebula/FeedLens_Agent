...
**直接调用现有 `get_context` / `add_memory`，不新建任何抽象层。**（✅ 已实现）

- `_build_planner_context` 中用状态摘要作 query 调 `get_context(query, n_episodic=10, n_long_term=3, lookback_days=7)`，结果注入上下文
  - 情节记忆（SQLite）：检索近 7 天执行记录，供 planner 回顾近期采集/排序/简报效果
...
- planner 上下文新增情节记忆（近 7 天执行记录，SQLite）和长期记忆（语义召回，ChromaDB）两个字段
- 每轮管线结束时多一次 `add_memory` 写入，长期记忆逐步积累
...
- **上下文压缩**：若情节记忆过长挤占 token，再考虑压缩。当前 `n_episodic=10, lookback_days=7` 足够精简
