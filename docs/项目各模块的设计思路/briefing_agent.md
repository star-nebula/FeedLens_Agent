...
Briefing Agent 的职责是：**接收排序后的条目列表，生成结构化 JSON 简报，并自动评估质量、必要时重试**。
...
| 决策 | 做法 | 理由 |
|------|------|------|
| **质量检查代码化** | 不暴露给 LLM 作为工具，代码层直接调用 | 消除 LLM 对质量判断的无效"思考"，避免循环 |
...
| **最多重试 2 次** | `generate_count >= max_retries` 即强制收敛 | 防止无限重试，保证响应时间可控 |
| **fallback 不报错** | JSON 解析失败时用原始数据直接拼接简报 | 保证一定有输出，不因 LLM 格式问题中断流程 |
...
```json
{
  ...
  "summary": "string        // 简报摘要，50字以内",
  "categories": [
    {
      "name": "string       // 分类名（如「科技」「商业」「社会」「其他」）",
      "count": "integer     // 该分类条目总数",
      "items": [
        {
          ...
          "final_score": "number   // 排序最终得分（回填强制覆盖）",
          "similar_count": "integer // 类似报道数量（仅主条目有，由代码回填）"
        }
        // items[0] 为主条目，items[1..] 为类似报道子条目
      ]
    }
  ],
...
| `final_score` | 覆盖（原始字段 `_score`） | 排序分数必须保留原始值 |
| `similar_count` | 代码计算后覆盖 | 由分组数据计算，不依赖 LLM |
...
> 预处理条目 → LLM 生成 JSON 简报 → 代码回填真实数据 → 代码自动打分 → 达标返回 / 不达标注入反馈重试（最多 2 次）→ 强制收敛。
