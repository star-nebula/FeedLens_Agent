...
> **注意**：v2.1.0 新增方案 07（向量预过滤）和 08（简报管线深度优化），详见对应 detail 文档。
> 
> - 07 方案 A：Collection Agent 内部预过滤（`collection_agent.py`），Planner 无感知
> - 08 方案 B：取消 LLM 自评，改为生成前硬编码预检 + 分离 coherence + quality 仅评 relevance + 降低重试上限
