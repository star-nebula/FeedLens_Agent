# FeedLens MVP 开发路线图 v2.0

> **版本**：v2.0 | **日期**：2026-06-18 | **说明**：优化阶段划分、职责边界和验证环节
...
阶段一 ──→ 阶段二 ──→ 阶段三 ──→ 阶段四 ──→ 阶段五 ──→ 阶段六
  骨架+基础设施     信息采集      排序+简报      推送+反馈      P1增强+UI     集成测试+交付
  数据模型          ReAct循环     ReAct循环      记忆管理       Streamlit     性能优化
```
...
- [ ] 创建项目目录结构（config / models / nodes / tools / utils / agents / mcp_server）
- [ ] 创建 `pyproject.toml` / `requirements.txt` 依赖管理文件
- [ ] 创建 `.env` 模板 + `config.py` 配置加载机制
...
- [ ] 4 个子 Agent StateGraph 骨架（CollectionAgent / RankingAgent / BriefingAgent / FeedbackAgent）
...
| 交付物 | 验收标准 |
|--------|---------|
| 项目目录结构 | 符合模块化设计，包含 agents / mcp_server 子目录 |
| 依赖管理文件 | `pip install -e .` 可安装成功 |
...
- [ ] SSE 连接断线降级逻辑
- [ ] 采集结果写入 SQLite（raw_items / feed_items 表）
...
| 数据写入 | 采集结果正确写入 raw_items 和 feed_items 表 |
...
- [ ] 时间衰减预筛 + Min-Max 归一化 + feedback_bias 逻辑
- [ ] `generate_briefing` FC 工具实现（沿用 category/importance/keywords）
- [ ] JSON → Markdown 渲染逻辑
...
| generate_briefing | 沿用 enrich_metadata 字段，items 按 category 分组 |
...
| 交付物 | 验收标准 |
|--------|---------|
| push_notification MCP | 推送服务作为子进程运行，端口管理清晰 |
...
- [ ] planner 自主编排能力增强（支持跳过采集/跳过简报/空数据回退等场景）
- [ ] 冷启动→偏好自适应切换完整链路（反馈数 >= 3 条）
- [ ] Streamlit 基础页面（首页 + Goal 设置 + RSS 管理 + 简报阅读 + 反馈 + 日志）
- [ ] 三级反馈 UI（like / dislike / irrelevant 按钮）
- [ ] 执行仪表盘（成功率、耗时、去重率、反馈率等指标）
...
| 优化项 | 原版本问题 | 优化方案 |
|--------|-----------|---------|
| 阶段划分 | 阶段四任务量过大（12个交付物） | 拆分为阶段四（推送+反馈+记忆）和阶段五（P1增强+UI） |
| 职责边界 | 去重功能放在阶段二，与排序Agent职责不一致 | 移至阶段三，与排序+简报形成完整处理链 |
| 基础设施 | 阶段一缺少依赖管理、环境配置 | 阶段一增加 `pyproject.toml`、`.env`、`config.py` |
| 验证环节 | 缺少阶段间验证 | 每个阶段增加验证环节（冒烟测试/功能测试） |
| 性能测试 | 性能测试放在最后，发现问题难回溯 | 阶段三完成后进行初步性能测试 |
| MCP 管理 | 两个MCP Server部署方式不明确 | 阶段一创建 mcp_server 目录，统一管理 |
